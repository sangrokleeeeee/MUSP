...
class VeriWild(BaseImageDataset):
    ...
    def __init__(self, root='datasets', verbose=True, test_size=3000, **kwargs):
        ...
        if self.test_size == 3000:
            self.test_query = osp.join(self.split_dir, 'test_3000_id_query.txt')
            self.test_list = osp.join(self.split_dir, 'test_3000_id.txt')
        elif self.test_size == 5000:
            self.test_query = osp.join(self.split_dir, 'test_5000_id_query.txt')
            self.test_list = osp.join(self.split_dir, 'test_5000_id.txt')
        elif self.test_size == 10000:
            self.test_query = osp.join(self.split_dir, 'test_10000_id_query.txt')
            self.test_list = osp.join(self.split_dir, 'test_10000_id.txt')
    ...
    def parse_img_pids(self, nl_pairs, pid2label=None):
        ...
        for info in nl_pairs:
            ...
            output.append([img_path, int(pid), int(camid)])
        return output
    ...
    def process_split(self, relabel=False):
        ...
        with open(self.train_list) as f_train:
            ...
            for data in train_data:
                img_path, pid, camid = data.strip().split()
                img_path = osp.join(self.img_dir, img_path)
                pid = int(pid)
                camid = int(camid)
                train.append([img_path, pid, camid])
                # train_pid_dict[pid].append([img_path, pid, camid])
