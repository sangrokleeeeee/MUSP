...
def re_ranking(input_feature_source, input_feature, k1=20, k2=6, lambda_value=0.1):
    ...
    if lambda_value != 0:
        ...
        sour_tar_dist = np.power(
            pairwise_distances(input_feature, input_feature_source, n_jobs=-1), 2).astype(np.float16)
        ...
        # all_num, all_num = 1, all_num + 
        source_dist = np.expand_dims(source_dist_vec, 0).astype(np.float64) +\
                      np.expand_dims(source_dist_vec, 1).astype(np.float64)
    ...
    original_dist = pairwise_distances(feat, feat, n_jobs=-1).astype(np.float16)  
